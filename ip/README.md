...
于是一个可以在公网上与其他设备相互通信的设备为局域网中使用私有地址的设备提供与公网的通信, 此设备即为 nat 路由器 (通常家用 wifi 路由器即为此类)

因私有地址不能出现在公网的 ip 报文中

具体而言

一个典型的使用 nat 与公网通信的客户端的路由表

default via 192.168.1.1 dev wlan0

192.168.1.0/24 dev wlan0 proto kernel scope link src 192.168.1.2

此处客户端地址为私有地址, 但其通信行为与公网上的设备相同

设 nat 设备的私有网段地址为 192.168.1.1/24 公网地址为 1.2.3.4/24

设 nat 设备的路由表

0.0.0.0/0 via 1.2.3.1

192.168.1.0/24 dev ...

1.2.3.0/24 dev ...

#### nat 设备为多个客户端提供同一公网地址访问公网

可多层嵌套

##### 当 192.168.1.2/24 发起一个向 8.8.8.8:53 tcp 连接

tcp 连接在同一套源 ip 源端口 目的 ip 目的端口只能有一个

设发送到 192.168.1.1 的 tcp 报文中源地址为 192.168.1.2:35000 目的地址为 8.8.8.8:53

nat 设备收到后, 将建立一个 1.2.3.4:53000 与 8.8.8.8:53 的 tcp 连接, 并保留 192.168.1.2:35000 与 1.2.3.4:53000 的映射关系, 8.8.8.8:53 向 1.2.3.4:53000 通过 tcp 发送的报文将被 nat 设备转发到 192.168.1.2:35000 (此处 35000 53000 为任意选取)

35000 与 53000 没有使用相同的数字是避免多个客户端同时以不同源 ip 相同源端口同时访问 8.8.8.8:53 将造成冲突, nat 设备应建立端口的映射

##### 关于 udp 仍然有端口概念可类比 tcp

##### ping 使用的 icmp 协议

客户端经常 ping 公网, 然而 icmp 运行在 ip 协议之上没有端口概念

但 icmp 报文中有 2 个字节的随机数作为 identify 回复报文中使用请求报文中的 identify

因此 nat 设备针对 identify 做映射

具体而言, nat 设备收到客户端的 icmp 的请求报文, 会读取其中的 identify(记为 A), 并用另一个 identify(记为 B) 替换, 并将 A 与 B 做映射

收到报文后选取 identify 为 B 的将 B 改为 A, 再发送给客户端, 需要转换映射 identify 是因为 nat 设备无法阻止多个客户端使用的 identify 向同一公网 ip 发送报文造成的冲突

通常而言 linux 中同一 ping 进程发送的报文 identify 相同

#### nat 设备为每一个客户端提供一个公网地址访问公网

一个常见的例子是云服务器

云服务商通常将临近的服务器置于同一网段中, 统一提供 dns 等, 而每个面向公网提供服务的服务器都应有公网地址
