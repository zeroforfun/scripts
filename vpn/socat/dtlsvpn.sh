# bash echo ip iptables jq killall openssl ping socat xargs
...
bash -c "/usr/bin/socat -T5 openssl-dtls-server:8443,cipher=ECDHE-RSA-AES256-GCM-SHA384,verify=1,commonname=client,cafile=/dev/shm/client.crt,certificate=/dev/shm/server.crt,key=/dev/shm/server.key,fork,reuseaddr tun,iff-up,tun-type=tap,tun-name=tap0 &" 0</dev/null 1>/dev/null 2>/dev/null
...
bash -c "/usr/bin/socat -T5 openssl-dtls-client:SERVERIP:8443,cipher=ECDHE-RSA-AES256-GCM-SHA384,verify=1,commonname=server,cafile=/dev/shm/server.crt,certificate=/dev/shm/client.crt,key=/dev/shm/client.key,forever tun,iff-up,tun-type=tap,tun-name=tap0 &" 0</dev/null 1>/dev/null 2>/dev/null
